...
class LogbookController extends Controller
{
    ...
    {
        $pengajuan = $this->findPengajuanForPembimbing($pengajuanId);
        $query = $this->logbookQueryForPembimbing()->where('pengajuan_id', $pengajuan->id);
        $this->applyFilters($query, $request);
    ...
    public function validasi(Request $request, $id)
    {
        ...
        $this->findPengajuanForPembimbing($logbook->pengajuan_id);
        ...
        if ($pengajuan?->mahasiswa?->user) {
            Notifikasi::create([
                ...
                'judul' => $request->status === 'revisi' ? 'Revisi Logbook dari Pembimbing Lapangan' : 'Logbook Disetujui Pembimbing Lapangan',
                'pesan' => 'Logbook tanggal ' . $logbook->tanggal . ' diberi status pembimbing lapangan: ' . $request->status . '. Catatan: ' . ($request->catatan ?? '-'),
                'status' => 'belum',
                'target_url' => route('mahasiswa.logbook.index', $pengajuan->id),
            ]);
        }

        return redirect()->back()->with('success', 'Status validasi pembimbing lapangan berhasil diperbarui.');
    }
    ...
    private function applyFilters($query, Request $request): void
    {
        ...
        if ($request->filled('status')) $query->where('status_mitra', $request->status);
    }

    private function findPengajuanForPembimbing($pengajuanId): PengajuanMagang
    {
        return PengajuanMagang::with(['mahasiswa.prodi', 'mitra', 'logbooks', 'periode', 'bimbingans.dosen', 'pembimbingLapangan'])
    ...
    private function findMissingWeeks(PengajuanMagang $pengajuan): array
    {
        ...
        for ($cursor = $start->copy(); $cursor->lessThanOrEqualTo($end); $cursor->addWeek()) {
            ...
            if (!isset($filledWeeks[$week])) $missing[] = $week;
        }
    ...
    private function combinedStatus(Logbook $logbook): string
    {
        ...
        if ($dosen === 'revisi' || $mitra === 'revisi') return 'revisi';
        if ($dosen === 'disetujui' && $mitra === 'disetujui') return 'disetujui';
        return 'pending';
    }
