            ...
            @forelse($pengajuans as $pengajuan)
                ...
                <tr>
                    ...
                    <td class="align-top">
                        ...
                        @if($sudahTerjadwal)
                            <button class="btn btn-sm btn-outline-primary" data-bs-toggle="collapse" data-bs-target="#jadwal-info-{{ $pengajuan->id }}">Lihat Jadwal</button>
                        @else
                ...
                <tr class="collapse" id="kelayakan-{{ $pengajuan->id }}"><td colspan="4">
                    <form action="{{ route('mahasiswa.seminar.kelayakan.store', $pengajuan->id) }}" method="POST" enctype="multipart/form-data" class="row g-3">
                        @csrf
                        <div class="col-12"><label class="form-label">Judul Laporan Magang</label><input type="text" name="judul_laporan" class="form-control" value="{{ old('judul_laporan', $pengajuan->judul_laporan) }}" required></div>
                        <div class="col-md-6"><label class="form-label">Laporan Hasil Magang Final (PDF)</label><input type="file" name="laporan_hasil_magang" class="form-control" accept=".pdf" required></div>
                        <div class="col-md-6"><label class="form-label">Produk Magang</label><input type="file" name="produk_magang" class="form-control" accept=".pdf,.zip,.rar,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.jpg,.jpeg,.png" required></div>
                        <div class="col-md-6"><label class="form-label">Draft Jurnal (PDF/DOC/DOCX)</label><input type="file" name="draft_jurnal" class="form-control" accept=".pdf,.doc,.docx" required></div>
                        ...
                        @endif
                        <div class="col-12 text-end"><button type="submit" class="btn btn-primary">Kirim/Ulang Kelayakan</button></div>
                    </form>
                </td></tr>
