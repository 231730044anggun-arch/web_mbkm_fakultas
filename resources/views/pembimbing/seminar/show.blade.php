...
<div class="card p-4 mb-4">
    ...
    <table class="table table-sm">
        <tr><th>Mahasiswa</th><td>{{ $kelayakan->pengajuan?->mahasiswa?->nama_lengkap ?? 'Belum tersedia' }} / {{ $kelayakan->pengajuan?->mahasiswa?->nim ?? 'Belum tersedia' }}</td></tr>
        <tr><th>Instansi</th><td>{{ $kelayakan->pengajuan?->mitra?->nama_instansi ?? $kelayakan->pengajuan?->nama_instansi_manual ?? 'Belum tersedia' }}</td></tr>
        <tr><th>Periode</th><td>{{ $kelayakan->pengajuan?->tanggal_mulai ?? 'Belum tersedia' }} s/d {{ $kelayakan->pengajuan?->tanggal_selesai ?? 'Belum tersedia' }}</td></tr>
    </table>
    ...
    <p><strong>Uraian Output Magang:</strong><br>{{ $kelayakan->output_magang ?: 'Belum tersedia' }}</p>
    <p><strong>Catatan Mahasiswa:</strong><br>{{ $kelayakan->catatan_mahasiswa ?: 'Belum tersedia' }}</p>
    @if($kelayakan->laporan_hasil_magang)
    ...
    @else
    <span class="badge bg-light text-dark border">Laporan belum tersedia</span>
    @endif
