...
<div class="card p-4 mb-4">
    <h6 class="fw-bold mb-3">Kelayakan Seminar Mahasiswa Bimbingan</h6>
        ...
        <table class="table table-hover align-middle">
            ...
            @forelse($kelayakans as $k)
                <tr>
                    ...
                    <td class="align-top"><a href="{{ route('dosen.seminar.show', $k->id) }}" class="btn btn-sm btn-outline-primary">Detail</a></td>
                </tr>
            @empty
...
<div class="card p-4">
    ...
    <table class="table table-hover align-middle">
        ...
        @forelse($pengajuans as $i => $p)
            <tr>
                <td class="align-top">{{ $i + 1 }}</td><td class="align-top">{{ $p->mahasiswa->nama_lengkap ?? '-' }}</td><td class="align-top">{{ $p->judul_laporan ?? '-' }}</td><td class="align-top">{{ $p->seminar_tanggal ?? '-' }}</td><td class="align-top">{{ $p->seminar_jam ?? '-' }}</td><td class="align-top">{{ $p->seminar_ruangan ?? '-' }}</td><td class="align-top"><span class="badge bg-secondary">{{ $formatStatus($p->status_seminar) }}</span></td><td class="align-top"><a href="{{ route('dosen.penilaian.create', $p->id) }}" class="btn btn-sm btn-outline-success">Nilai</a></td>
            </tr>
        @empty
